...
public class NavMeshAI_V16 : MonoBehaviour {
	...
	public bool hasJustLostPlayer = false;
	...
	public int playerSoundDistance; //default set to 3;
	...
	public void searchPlayer()
	{
		...
		Ray lineToPlayer = new Ray (rayCastStart.position, (playerPosition.position-rayCastStart.position));
		RaycastHit hit;
		//Debug.DrawRay (transform.localPosition, transform.forward, Color.blue, 20f);
		Debug.DrawRay (rayCastStart.position, Vector3.ClampMagnitude((playerPosition.position - rayCastStart.position), distanceToPlayer), Color.green, 0.1f);
		
		if(Physics.Raycast(lineToPlayer, out hit, distanceToPlayer))
		{
			Debug.Log (hit.collider.tag);
			if(hit.collider.tag == "Player" && playerPosition.position.y < 6)
			{
	...
	{
		/*if(playerPosition.position.y > 6)
		{
			Debug.Log ("Player is too high");
			hasJustLostPlayer = true;
			animator.ResetTrigger("SpotPlayer");
			animator.ResetTrigger("Attack");
		...
		{
			RaycastHit hit;
			if(Physics.Raycast (rayCastStart.position, (playerPosition.position-rayCastStart.position), out hit, Mathf.Infinity))
			{
	...
	public void runningPlayer()
	{
		...
		{
			if(Mathf.Abs(playerPosition.position.x - transform.position.x) < playerSoundDistance && Mathf.Abs(playerPosition.position.z - transform.position.z) < playerSoundDistance)
			{
	...
	{
		Debug.Log ("Trying to find Sound");
