...
public class Boss_AI_V2 : MonoBehaviour {
	...
	//public float throwForce; //default set to 2
	public GameObject gameObjectRock;
	public RockCollision rockCollision;
	public bool isThrowLeftRock = false;
	public bool isThrowRightRock = false;

	//For going to the destination: Follow player if he throws rocks and then the player is still in his line of sight
	...
	void Start () 
	{
		...
		rockCollision = gameObjectRock.GetComponent<RockCollision> ();
	...
	{
		/*
		if(hasLeftRock == true || hasRightRock == true)
		{
			Debug.Log("Already Has Rocks");
			//proceed to throw rocks 
			state = 3;
		}
		*/
		...
		//timer so that animation can be done
	...
	{
		rightRock = (GameObject) Instantiate (rock, rightHand.transform.position , transform.rotation);
		rightRock.transform.parent = rightHand.transform;
		//FOR TEST USE THESE COORDINATES
		rightRock.transform.localPosition = new Vector3 (0, 0, 1);
		//FOR REAL SPECTRAL, USE 
		//rightRock.transform.localPosition = new Vector3 (0, 0, 0);
		
		leftRock =  (GameObject) Instantiate(rock, leftHand.transform.position, transform.rotation);
		leftRock.transform.parent = leftHand.transform;
		//FOR TEST USE THESE COORDINATES
		leftRock.transform.localPosition = new Vector3 (0, 0, 1);
		//FOR REAL SPECTRAL, USE:
		//leftRock.transform.localPosition = new Vector3 (0, 0, 0);
	}
	...
	{
		//turns the spectral to look at the player 
		AIAgent.Stop();
		Quaternion fromRotation = Quaternion.LookRotation(transform.forward);
		...
		if (hasLeftRock) 
		{
			...
			if (throwTimer >= throwTime * 2) {
				if (hasLeftRock) {
					throwLeftRock ();
					//isThrowLeftRock = false;
				}
			if(throwTimer >= throwTime *2.5)
				{
		...
		{
			AIAgent.Resume();
			Debug.Log ("Tried to throw rocks, no Rocks");
			state = 0;
		}
	}
	public void throwLeftRock()
	{
		...
		//leftRockRigidBody.MovePosition (playerPosition.position);


		Vector3 throwVector = transform.forward + new Vector3 (playerPosition.position.x-transform.position.x, 5, playerPosition.position.z-transform.position.z);
		leftRockRigidBody.velocity = throwVector;

		//leftRockRigidBody.AddForce (throwVector * 180);
	}

	public void throwRightRock()
	{
		hasRightRock = false;
		//isThrowRightRock = true;
		...
		//rightRockRigidBody.MovePosition (playerPosition.position);

		Vector3 throwVector = transform.forward + new Vector3 (playerPosition.position.x - transform.position.x, 5, playerPosition.position.z-transform.position.z);
		rightRockRigidBody.velocity = throwVector;
		//rightRockRigidBody.AddForce (throwVector * 180);
